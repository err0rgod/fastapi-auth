...
def test_userdata_creation():
    ...
    user = userdata(user_id="test-123", user_name="testuser", password="hashedpassword")
...
def test_jwt_blacklist_creation():
    ...
    blacklist = jwt_blacklist(user_name="testuser", jti="uuid-token-id", expired_at=datetime.now())
...
def test_refresh_session_creation():
    ...
    session = refreshSession(
        session_id="sess-123", user_id="test-123", token_hash="hashabc", expires_at=datetime.now()
    )
